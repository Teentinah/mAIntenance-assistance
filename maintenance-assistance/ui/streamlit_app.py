...
SCENARIO_BADGE = {
    "incident_courant": ("Scénario 1", "badge-success"),
    "incident_urgent": ("Scénario 2", "badge-danger"),
    "demande_incomplete": ("Scénario 3", "badge-warning"),
    "demande_sensible": ("Scénario 4", "badge-danger"),
}
...
if "pending_queue" not in st.session_state:
    st.session_state.pending_queue = [dict(t) for t in get_store().tickets_queue]


def _process_and_record(ticket_id: str, texte: str) -> None:
...
def render_stats():
    ...
    cards = [
        ("File d'attente", len(st.session_state.pending_queue), "tickets déjà reçus", "var(--ispm-amber)"),
...
def render_scenario_guide():
    ...
    st.caption("Traitez un ticket de la file d'attente ci-dessous (badges Scénario 1 à 4), ou saisissez-en un nouveau.")
    cols = st.columns(4)
...
def render_queue():
    ...
    with header_col:
        _md(f'<div class="ispm-card" style="margin-bottom:0;"><h4>File d\'attente — {len(queue)} ticket(s) déjà reçu(s), en attente d\'analyse</h4></div>')
    with reset_col:
        if st.button("↺ Réinitialiser la file", use_container_width=True):
            st.session_state.pending_queue = [dict(t) for t in get_store().tickets_queue]
            st.rerun()

    if not queue:
        st.success("File d'attente vide : tous les tickets reçus ont été traités. Vous pouvez la réinitialiser ou saisir un nouveau ticket ci-dessous.")
        return

    for t in queue:
        badge_html = ""
        if t.get("scenario") in SCENARIO_BADGE:
            label, css = SCENARIO_BADGE[t["scenario"]]
            badge_html = f'<span class="badge {css}">{label}</span> '
        _md(f"""
            <div class="ispm-card" style="margin-bottom:0.6rem; padding:0.9rem 1.1rem;">
            <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:1rem;">
              <div style="flex:1;">
                <div style="font-size:0.75rem; color:var(--ispm-gray); margin-bottom:2px;">
                  {badge_html}<b>{t['ticket_id']}</b> · reçu {t.get('soumis_le', '')} via {t.get('canal', 'portail')}
                </div>
                <div style="font-size:0.9rem; color:var(--ispm-charcoal);">{t['texte']}</div>
              </div>
            </div>
            </div>
        """)
        ...
        with btn_col:
            if st.button("Traiter ce ticket", key=f"traiter_{t['ticket_id']}", use_container_width=True):
                _process_and_record(t["ticket_id"], t["texte"])
                st.session_state.pending_queue = [
                    q for q in st.session_state.pending_queue if q["ticket_id"] != t["ticket_id"]
                ]
                st.rerun()
...
def page_nouveau_ticket():
    ...
    _md('<div class="ispm-card" style="margin-top:0.8rem; margin-bottom:0;"><h4>Ou saisir un nouveau ticket</h4></div>')
    texte = st.text_area(
    ...
    if analyser:
        if not texte or not texte.strip():
            ...
            _process_and_record(_next_ticket_id(), texte)
